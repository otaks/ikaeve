        ...
        <div class="card-body">
            <div class="container-fluid">
                <table class="table table-bordered">
                    ...
                    <tr><th>フレンドコード</th><td>
                      @if($data->friend_code) {{ substr($data->friend_code, 0, 4) }}-{{ substr($data->friend_code, 4, 4) }}-{{ substr($data->friend_code, 8, 4) }}-{{ substr($data->friend_code, 12, 4) }} @endif</tr>
                    ...
                    @foreach($data::members($data->id) as $v => $member)
                      <tr><th>メンバー{{($v+1)}}</th><td><a href="{{ route('member.detail', ['id' => $member->id]) }}">{{ $member->name }}</a>
                        ...
                        @endif
                        @if($member->discord)
                            &nbsp;<a href="https://twitter.com/{{ $member->discord }}" target="_blank"><i class="fab fa-discord fa-2x"></i></a>
                        @endif</tr>
                    @endforeach
